...
export default function LegacyErrorPage({ status }: { status: 403 | 404 | 500 }) {
  ...
  return <main className={`legacy-error legacy-error-${status}`}><Card className="legacy-error-card" bordered={false}><Result status={status} icon={<div className="error-orbit"><strong>{status}</strong></div>} title={copy[0]} subTitle={copy[1]} extra={<Space wrap><Button type="text" className="legacy-back-button legacy-back-button-result" icon={<ArrowLeftOutlined />} href="/">返回首页</Button>{status === 500 && <Button icon={<ReloadOutlined />} onClick={() => window.location.reload()}>重新加载</Button>}</Space>} /></Card></main>
}
