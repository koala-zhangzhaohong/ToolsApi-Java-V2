...
export default function LegacySearchPage() {
  ...
  const searchBox = <>
    <form className="legacy-search-form" onSubmit={(event) => { event.preventDefault(); void search() }}>
      ...
      <Button className="legacy-search-submit" type="primary" htmlType="submit" size="large" disabled={loading} aria-label={loading ? '正在解析' : '立即解析'}>
        <span className="search-button-content">{loading ? <LoadingOutlined className="legacy-search-button-loading" /> : <SearchOutlined />}<span className="search-button-label">立即解析</span></span>
      </Button>
    </form>
