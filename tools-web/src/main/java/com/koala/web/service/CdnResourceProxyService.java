...
public class CdnResourceProxyService {
    ...
    public void redirect(HttpServletResponse response, String cdnUrl) throws IOException {
        cdnUrl = normalizePublicCdnUrl(cdnUrl);
    ...
    /** Keeps redirects generated by older nodes/Redis entries compatible with Traefik's public route. */
    public String normalizePublicCdnUrl(String url) {
        if (!StringUtils.hasText(url)) return url;
        return url.replaceFirst("(?i)^(https?://[^/]+)/doProxy(?=\\?|$)", "$1/proxy/doProxy");
    }
